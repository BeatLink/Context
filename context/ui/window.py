...
class LauncherWindow(Gtk.ApplicationWindow):
    ...
    def refresh(self) -> None:
        ...
        # How many scratchpads are on offer, which is the one thing a folded
        # section can still say about itself.
        pads = sum((bool(live.scratchpad_global), bool(live.scratchpad_per_context)))
        self.notes_label.set_label(f"Scratchpad · {pads}" if pads else "Scratchpad")

        self.open_label.set_visible(bool(opened or loose))
        self.open_listbox.set_visible(bool(opened or loose))
        # Counted like every other group. The no-context is one of them: it is
